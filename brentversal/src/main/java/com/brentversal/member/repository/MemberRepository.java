...
public interface MemberRepository extends JpaRepository<Member, Long> {
    ...
    // ── 관리자 홈 "회원 통계" 에서 쓰는 집계 ──────────────────────

    // 역할별 회원 수 (일반 사용자 / 중개인 / 관리자 비중)
    long countByRole(Role role);

    // 이번 달 신규 가입자 수
    long countByRegdateGreaterThanEqual(LocalDate from);

    /*
      월별 가입자 추이. 집계를 DB 에 맡긴다. 회원을 전부 읽어 와서 자바에서 세면
      회원이 늘어날수록 읽는 양이 그대로 늘어나기 때문이다.
      반환값은 [연, 월, 건수] 순서의 Object[] 목록이다.
    */
    @Query("select year(m.regdate), month(m.regdate), count(m) " +
           "  from Member m " +
           " where m.regdate >= :from " +
           " group by year(m.regdate), month(m.regdate)")
    List<Object[]> countMonthlySignups(@Param("from") LocalDate from);
}
